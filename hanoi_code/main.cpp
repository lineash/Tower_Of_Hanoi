...
{
    //버튼 초기화
    hanoi_selectButton[hanoi_tempTower]->setImage("hanoi_selectButton.png"); //ing->select 버튼 이미지 변경
...
void hanoi_create_scene2()
{
    ...
    for(int i =0; i<3; i++)
    {
        hanoi_selectButton[i] = Object::create("hanoi_selectButton.png", hanoi_scene2, 290+300*i, 130);
    }
    for(int i =0; i<3; i++)
    {
        hanoi_toButton[i] = Object::create("hanoi_toButton.png", hanoi_scene2, 290+300*i, 130);
    ...
    for(int i = 0; i<3; i++)
    {
        hanoi_selectButton[i]->setOnMouseCallback([&,i](ObjectPtr object, int x, int y, MouseAction action)->bool{
            ...
            {
                hanoi_selectButton[i]->setImage("hanoi_ingButton.png"); //버튼 이미지 변경
                for(int button = 0; button <3; button++) //버튼 변경
                {
                    if(button!=i)
                    {
    ...
    for(int i = 0; i<3; i++)
    {
        hanoi_toButton[i]->setOnMouseCallback([&, i](ObjectPtr object, int x, int y, MouseAction action)->bool{
            ...
            {
                hanoi_clearButton=Object::create("hanoi_clearButton.png",hanoi_scene2, 540, 550);
...
{
    //씬 생성
    hanoi_scene1 = Scene::create("scene1", "hanoi_scene1.png");
    hanoi_scene2 = Scene::create("scene2", "hanoi_scene2.png");
    
    //오브젝트 생성, 콜백 등록
    hanoi_startButton = Object::create("hanoi_startButton.png", hanoi_scene1, 880, 200);
    ...
    //시작화면-endButton
    hanoi_endButton = Object:: create("hanoi_endButton.png", hanoi_scene1, 880, 100);
    ...
    for(int i=0; i<3; i++)
    {
        Object::create("hanoi_tower.png", hanoi_scene2, 240 + 300*i, 190.8);
    }
